...
void tampilkanKatalog(mesin dataMesin[], int jumlahMesin){
    ...
    for(int i=0;i<jumlahMesin;i++){
        cout<<left<<setw(5)<<i+1 <<setw(25)<<dataMesin[i].nama_mesin <<setw(15)<<dataMesin[i].harga_perjam <<setw(10)<<dataMesin[i].ketersediaan_jumlah <<setw(18)<<dataMesin[i].dataProdusen.perusahaan <<setw(10)<<dataMesin[i].dataProdusen.tahun_pembuatan <<endl;
    }
}
...
int hitungTotalMesin(mesin dataMesin[], int hitung){
    ...
    return dataMesin[hitung].ketersediaan_jumlah + hitungTotalMesin(dataMesin,hitung-1);
}
...
int main(){
    ...
    do{
        ...
        cin>>pilihanMenu;
        if(pilihanMenu==1){
            registrasiMesin(dataMesin,jumlahMesin);
        }
        else if(pilihanMenu==2){
            tampilkanKatalog(dataMesin,jumlahMesin,"DATA KATALOG");
        }
        else if(pilihanMenu==3){
            updateJumlahMesin(dataMesin,jumlahMesin);
        }
        else if(pilihanMenu==4){
            hapusMesin(dataMesin,jumlahMesin);
        }
        else if(pilihanMenu==5){
            int totalMesin = hitungTotalMesin(dataMesin,jumlahMesin-1);
